...
- Enforce the published Forecast Ledger v1.1.0 model: globally unique stable IDs, append-only forecast history, flexible empty ledger/question states, typed question/value/resolution agreement, lifecycle chronology, exact schema validation, and format-preserving transactions. The `adopt-forecast-ledger-v1-1` change supersedes this change's earlier v1.0.0 minimum-item assumptions.
...
- Close the remaining v0.2.2 dogfooding boundary gaps with aggregate target inspection, one recorded-time default, accurate temporal and source-location diagnostics, consistent inserted-field order, actionable MCP root errors, an explicit immediate-lock-contention contract, and locally visible retained Bitcoin anchoring details.
