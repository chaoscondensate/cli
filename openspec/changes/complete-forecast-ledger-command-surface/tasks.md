...
- [x] 17.6 Extend forecast and verification public DTOs and human/plain presenters with safe retained integrity detail, including receipt state, Bitcoin block height, anchored-before time, and verification time; label offline values as stored/not freshly rechecked and add no-network and secret-redaction goldens.
- [x] 17.7 Update README, CLI help, command reference, authoring/target/MCP/timestamp/verification guides, and release notes for the unified init time default, inclusive boundary wording, target-check result states, immediate lock conflict and caller retry/backoff, actionable root errors, and locally visible Bitcoin evidence.
...
- [ ] 17.9 Run the new fixtures against packaged macOS, Linux, and Windows binaries, preserve the independently verified seal/OTS evidence as regression references without making external live services deterministic test dependencies, and record patch-release evidence before closing tasks 16.12 and 15.8.
