...
- **WHEN** a user runs `forecast-ledger platform add --file l.yaml --platform metaculus --name Metaculus --kind scoring_platform` against a valid ledger and omits `--input`
...
- **THEN** the command constructs the same typed application request and produces the same validated ledger state as the equivalent optional input document
...
An authoring command MAY retain `--input <path>` or `--input -` as an optional compatibility and batch-input mode. Flag mode and input-document mode SHALL be mutually exclusive except for selectors, output controls, dry-run controls, and secret-file destinations that are outside the document schema. Supplying `--input` together with any document-mapped authoring flag SHALL fail as usage before reading secrets or changing files. Omitting `--input` SHALL never fail merely because the command is in flag mode.

#### Scenario: Flag-only mode succeeds
- **WHEN** all fields required for the selected operation are supplied by flags and `--input` is absent
- **THEN** validation proceeds without an input-source error

#### Scenario: Optional document mode remains compatible
- **WHEN** a user supplies one valid input document and no document-mapped authoring flags
- **THEN** the command accepts the document under the existing bounded closed-schema parsing rules

#### Scenario: Mixed authoring sources are rejected
- **WHEN** a user supplies `--input` and a flag that maps to a field inside that input
- **THEN** the command returns a usage error identifying the conflicting sources and performs no mutation
...
Flag mode and optional document mode SHALL converge on the same transport-neutral typed application request before validation, locking, cryptography, or mutation. They SHALL produce the same stable success result, error codes, dry-run effects, source-preserving write behavior, and atomicity for equivalent values. CLI parsing logic MUST NOT implement a second domain mutation path.

#### Scenario: Equivalent inputs have parity
- **WHEN** equivalent non-secret values are supplied once by flags and once by an optional input document against equivalent ledgers
