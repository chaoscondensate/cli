...
func TestUnmarshalIndividualFixtureIntoTypedModel(t *testing.T) {
	data, err := os.ReadFile(filepath.Join("..", "schema", "testdata", "forecast-ledger", "v1.0.0", "individual-ledger.json"))
	if err != nil {
		t.Fatal(err)
	}
	...
	if got.SchemaVersion != "1.0.0" || got.LedgerID != "alex-example-forecasts" {
