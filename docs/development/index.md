# Development documentation

<!-- doc-metadata
coverage: development
reviewed: 2026-08-25
owner: documentation
generated: false
security-critical: false
prerequisites: ../../AGENTS.md
next: build.md
-->

- [Build the CLI](build.md)
- [Review dependencies](dependencies.md)
- [Review product names](name-review.md)
- [Create and verify releases](releasing.md)
- [Review the documentation baseline](documentation-baseline.md)
- [Track the OpenTimestamps release review](ots-review.md)
- [Write and review documentation](documentation-style.md)
- [Apply page metadata and ownership](page-metadata.md)
- [Create accessible visual assets](visual-style.md)
- [Check licensing compliance](licensing.md)
- [Trace documentation requirements](documentation-traceability.md)

Contributors and coding agents must also read the repository
[contributor context](../../AGENTS.md) before changing behavior.

[Documentation index](../index.md)
